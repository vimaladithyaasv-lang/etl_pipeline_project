...
Responsibilities:
  ...
  - Write clean tables back to staging.db ready for the load layer.
"""
import logging
import re
import sqlite3
...
from etl.config import (
    ...
    COUNTRY_MAP, CATEGORY_MAP, STATUS_MAP, KNOWN_CATEGORIES,
)
...
def transform_products(sconn: sqlite3.Connection,
    ...
    # --- Quarantine: duplicate product_id
...
def transform_orders(sconn: sqlite3.Connection,
    ...
    # Parse amount; recompute if missing
    ...
    # Recompute missing amount from price × quantity using product lookup
    price_map = dict(pd.read_sql(
        "SELECT product_id, price FROM stg_products", sconn
    ).values)
    missing_amt = df["amount"].isna()
    if missing_amt.any():
        df.loc[missing_amt, "amount"] = df.loc[missing_amt].apply(
            lambda r: _parse_price(price_map.get(r["product_id"], None))
                      * r["quantity"]
            if price_map.get(r["product_id"]) else None,
            axis=1,
        )

    # Drop rows where amount still missing after recompute
...
def run_transform() -> dict:
    ...
    orders     = transform_orders(sconn, rconn, valid_cids, valid_pids)
